...
namespace AtlusScriptLibrary.MessageScriptLanguage.Compiler
{
    ...
    public class MessageScriptCompiler
    {
                        ...
                        case "f":
                            {
                                ...
                                {
                                    mLogger.ConsoleError( "Failed to compile function token" );
                                    return false;
                                }
                        ...
                        case "x":
                            {
                                ...
                                {
                                    mLogger.ConsoleError( "Failed to compile code point token" );
                                    return false;
                                }
        ...
        {
            mLogger.ConsoleInfo( $"({context.Start.Line:D4}:{context.Start.Column:D4}) {str}" );
        }

        private void LogInfo(string str )
        {
            mLogger.ConsoleInfo( str );
        }

        private void LogError( ParserRuleContext context, string str )
        {
            mLogger.ConsoleError( $"({context.Start.Line:D4}:{context.Start.Column:D4}) {str}" );
        }

        private void LogError( Antlr4.Runtime.IToken token, string str )
        {
            mLogger.ConsoleError( $"({token.Line:D4}:{token.Column:D4}) {str}" );
        }
        ...
        private class AntlrErrorListener : IAntlrErrorListener<Antlr4.Runtime.IToken>
        {
            ...
            {
                mCompiler.mLogger.ConsoleError( $"Syntax error: {msg} ({offendingSymbol.Line}:{offendingSymbol.Column})" );
            }
        }
    }
