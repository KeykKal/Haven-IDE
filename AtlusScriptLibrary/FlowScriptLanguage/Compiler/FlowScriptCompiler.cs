        ...
        private bool TryResolveProcedureLabels( out List<FlowScriptLanguage.Label> labels )
        {
            ...
            if ( mLabels.Values.Any( x => !x.IsResolved ) )
            {
                foreach ( var item in mLabels.Values.Where( x => !x.IsResolved ) )
                    mLogger.ConsoleError( $"Label '{item.Name}' is referenced but not declared" );

                mLogger.ConsoleError( "Failed to compile procedure because one or more undeclared labels are referenced" );
                labels = null;
                return false;
            }
        ...
        private void TraceInstructionStackBehaviour( Instruction instruction )
        {
            switch ( instruction.Opcode )
            {
                ...
                case Opcode.END:
                {
                    ...
                    mLogger.ConsoleDebug( $"{mStackValueCount} values on stack at END" );

                    if ( mStackValueCount < 1 )
                    {
                        mLogger.ConsoleError( "Stack underflow!!!" );
                    }
                    else if ( mStackValueCount != 1 )
                    {
                        mLogger.ConsoleError( "Return address corruption" );
                    }
                }
                    break;
                case Opcode.ADD:
